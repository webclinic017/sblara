...
class SectorGainerLoser
{
    ...
    public function compose(View $view)
    {
       ...
       // dd($instrumentList->first()->sector_list->name);

        $instrumentList=$instrumentList->groupBy('sector_list_id');
        //dump($instrumentList->groupBy('sector_list_id')->toArray());
        $upDownData=DataBanksIntradayRepository::upDownStats();
        $sector_wise_up=array();
        $sector_wise_down=array();
        $sector_wise_eq=array();
